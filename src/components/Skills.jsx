      ...
      <div className="space-y-8 mt-6">
        {Object.entries(skillsByCategory).map(([category, categorySkills]) => (
          <div key={category} className="space-y-4">
            <h3 className={`text-lg font-semibold bg-gradient-to-r ${categoryColors[category]} bg-clip-text text-transparent drop-shadow-sm`}>
              {category}
            </h3>
            <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {categorySkills.map((skill, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="flex flex-col items-center text-center p-3 rounded-2xl shadow-lg backdrop-blur-md bg-stone-100/30 dark:bg-gray-800/50 hover:scale-105 hover:shadow-xl transition duration-300 ease-in-out border border-gray-200/20 dark:border-gray-700/50 hover:border-gray-300/30 dark:hover:border-gray-600/50"
                  >
                    <span className="p-3 sm:p-4 rounded-xl bg-gradient-to-br from-white/90 to-gray-100/90 dark:from-gray-700 dark:via-gray-800 dark:to-gray-900 shadow-inner border border-gray-200/30 dark:border-gray-600/30">
                      <Icon 
                        size={25} 
                    ...
                    <span className="text-xs sm:text-sm mt-2 font-medium text-gray-700 dark:text-gray-200">
                      {skill.name}
                    </span>
                  </div>
                );
